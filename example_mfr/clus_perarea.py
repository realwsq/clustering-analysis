...
if "IC" in inc_param and inc_param['IC']:
    data = pickle.load(open("./example_mfr/utils/CTreduced_for_shuqi.pck", 'rb'))
else:
    data = pickle.load(open("./example_mfr/utils/CT_for_shuqi.pck", 'rb'))
...
for area in tqdm(conn_area_list_byH):
    ...
    plot_kwargs['vs'] = [f"{c}" for c in conditions] # add vs for plot
    ...
    if clus_res['final_clus_res']['clus_success']: 
        ...
        for li in np.unique(_labels):
            ...
            # Xs = clus_res['data_clus_hist'][-1]['X_orig']
            Xs_li = Xs[_labels==li]
